...
getAttribute str (Element _ attrs  _)
  | attrs' == [] = []
  | otherwise    = snd (head attrs')
  where
   attrs' = filter ((== str) . fst) attrs
getAttribute _ _
  = []
...
getValue (Element _ _ xmls)
  = Text (concatMap getValueH xmls)
  where
   getValueH Null               = []
   getValueH (Text str)         = str
...
splitCon f g as
  = (takeWhile f as, g (dropWhile f as))
...
expandXSL' (Element dir attrs sources) xsl@(Element name [(a, str)] [])
  = case name of
    "value-of" -> case ts of
      [] ->  case (head h) of
        '@' -> [Text (head (map (getAttribute (tail h)) sources))]
        _   -> [get (map (getValue) sources)]
      _  -> expandXSL' (Element h attrs sources') xsl'
  where
   xsl'     = Element name [(a, ts)] []
   (h, ts)  = splitCon (/= '/') (dropWhile (flip elem "/.")) 
                           (dropWhile (flip elem "/.") str)
   sources' 
     | head ts == '@' = sources
     | otherwise      = concatMap (getChildren (takeWhile (/= '/') ts)) sources
   get []     = Text []
   get (x:xs) = x
